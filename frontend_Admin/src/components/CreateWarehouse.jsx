...
function CreateWarehouse(props) {
    ...
    const handleSubmit = async (e) => {
        ...
        const _products = data.products.map(item => ({ ...item, quantity: quantity[item.productId] || 0 }));
        ...
        <form className="w-full block">
            <div className="grid md:grid-cols-2 gap-12 dark:bg-darkPrimary rounded-lg">
                ...
                <div className="w-full flex flex-col dark:bg-black bg-white rounded gap-2 text-balck dark:text-white p-4">
                    ...
                    {
                        ploading ? <span>Loading...</span> : products.length > 0 ? products.map((prod, index) => <div className="w-full border-white rounded-md border py-2 px-4 bg-neutral-100 dark:bg-darkPrimary flex items-center justify-between" key={index}>
                        ...
                        </div>) : <div className="">No Products Found</div>
                    }
                </div>
