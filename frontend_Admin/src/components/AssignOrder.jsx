...
function AssignOrder() {
    ...
    const handleSubmit = async () => {
        ...
        if (editdata?.payment_status == "others" && editdata?.otherpayment_status?.trim() == "") {
        ...
        if (editdata?.platform === "others" && editdata?.other_platform?.trim() === "") {
        ...
        try {
            ...
            if (response.data.success) {
                alert(response.data.message + ` Order id :- ${response.data?.order_id}`);
                navigate("/");
            } else {
                toast.error(response.data.message);
            }
    ...
    return (
        <div className="dark:bg-black bg-white sm:px-8 p-4">
            <div className="flex w-full sm:item-center sm:justify-between gap-2 sm:flex-row flex-col mb-6">
                ...
                <span className="text-lg font-bold inline-block dark:text-gray-100">#{order?.order_number}</span>
            </div>
            <div className="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100  lg:mb-16 mb-7 ">
                <div className="container mx-auto p-6">
                    ...
                    {/* Warehouse Section */}
                    <div className="mb-6 p-4 bg-white dark:bg-gray-800 rounded-md shadow-md">
                        <h2 className="text-xl font-semibold mb-2">Warehouse</h2>
                        <p><strong>Name:</strong> {order?.warehouse_id?.name}</p>
                        <p><strong>Address:</strong> {order?.warehouse_id?.address}</p>
                        <p><strong>Email:</strong> {order?.warehouse_id?.email}</p>
                    </div>

                    {/* Customer Section */}
                    <div className="mb-6 p-4 bg-white dark:bg-gray-800 rounded-md shadow-md">
                        <h2 className="text-xl font-semibold mb-2">Customer</h2>
                        <p><strong>Name:</strong> {order?.customer?.fname} {order?.customer?.lname}</p>
                        <p><strong>Email:</strong> {order?.customer?.email}</p>
                        <p><strong>Phone:</strong> {order?.customer?.phone}</p>
                        <p><strong>Address:</strong> {order?.customer?.address}, {order?.customer?.city}, {order?.customer?.state}, {order?.customer?.country}</p>
                    </div>
            ...
            <div className="flex w-full items-start lg:flex-row flex-col md:gap-8 gap-4">
                <div className="w-[200px]">
                    <div className="text-[#3c98d6] text-xl font-medium font-['Inter']">Edit Order</div>
                ...
                <div className="w-full">
                    <div className="flex items-start md:flex-row md:gap-7 gap-6 w-full">
                        <div className="space-y-4 w-full">
                            <div className="form-control">
                                ...
                                <select
                                    ...
                                    value={editdata?.payment_method || order?.payment_method || ""}
                        ...
                        <div className="space-y-4 w-full">
                            ...
                            {editdata?.payment_status == "others" && <div className="form-control pt-4">
            ...
            <div className="flex w-full items-start lg:flex-row flex-col md:gap-8 gap-4">
                <div className="w-[200px]">
                    <div className="text-[#3c98d6] text-xl font-medium font-['Inter']">Edit Shipping Partner Details</div>
                </div>
                <div className="w-full">
                    <div className="dark:text-white text-black">
                        <div className="grid md:grid-cols-2 md:gap-9 gap-5">
                            ...
                            {editdata?.platform == "others" && <div className="form-control pt-4">
                                <label htmlFor="otherpayment_status" className="block md:text-[20px] text-[15px] dark:text-white text-black font-['Inter'] mb-2">
            ...
            <div className="flex w-full items-start lg:flex-row flex-col md:gap-8 gap-4 pt-6 sm:pt-4">
                <div className="w-[200px]">
                    <div className="text-[#3c98d6] text-xl font-medium font-['Inter']">Order Specification</div>
                        ...
                        <div className="grid md:grid-cols-2 md:gap-9 gap-5">
                            ...
                            <div className="w-full md:col-span-2">
                                ...
                                <input
                                    ...
                                    value={((dimensions.breadth) * (dimensions.height) * (dimensions.length)) / 5000}
                                    className="w-full md:py-4 py-2.5 md:px-6 px-4 dark:text-white text-black text-sm dark:bg-[#121212] bg-gray-300  shadow-sm rounded-md"
                                />
                            </div>
                        </div>
