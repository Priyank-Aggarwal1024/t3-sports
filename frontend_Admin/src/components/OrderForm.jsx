...
const OrderForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedWarehouse) {
      setValidationMessage(
        "Please select a warehouse before submitting the order."
      );
      return;
    }
    if (
      !orderDetails.payment_method ||
      orderDetails.payment_method.trim() == ""
    ) {
    ...
    if (
      ...
      orderDetails.otherpayment_status.trim() == ""
    ) {
      setValidationMessage(
    ...
    if (!selectedCustomer) {
      setValidationMessage(
        "Please select a customer before submitting the order."
      );
      return;
    }
    ...
    if (
      ...
      shippingDetails.shippingAddress.country.trim() == ""
    ) {
    ...
    if (
      orderDetails.platform === "others" &&
      orderDetails.other_platform.trim() === ""
    ) {
  ...
  return (
    <div className="dark:bg-black bg-white">
      ...
      >
              ...
              <div className="grid md:grid-cols-2 md:gap-9 gap-5">
                <div className="w-full">
                  ...
                  <select
                    ...
                    name={"shipping_type"}
                    onChange={({ target }) =>
                      handleOrderDetailChange(target.name, target.value)
                    }
          ...
          <div className="w-full">
            ...
            <div className="w-full pt-10">
              <label
                ...
                className="block md:text-[20px] text-[15px] dark:text-white text-black font-['Inter'] cursor-pointer md:mb-6 mb-4"
              >
                Insurance Required
              </label>
              <div className="flex items-center gap-6">
                {orderDetails.insuranceRequired ? (
                  <span
                    className="md:w-8 md:h-8 h-6 w-6 flex justify-center items-center rounded-md md:text-2xl text-xl dark:text-black cursor-pointer text-white dark:bg-white bg-black"
                    onClick={handleInsuranceChange}
                  >
                    ✔
                  </span>
                  ...
                  <span
                    className="block md:w-8 md:h-8 h-6 w-6 rounded-md md:text-2xl text-xl dark:bg-white bg-black cursor-pointer"
                    onClick={handleInsuranceChange}
                  ></span>
                )}
                <div className="dark:text-white text-black text-md md:text-xl font-normal font-['Inter']">
                  {orderDetails.insuranceRequired ? "Yes" : "No"}
                </div>
              </div>
            </div>
            <div className="w-full dark:bg-[#8A8A8A] bg-gray-600 lg:my-16 my-8 h-[0.5px]"></div>
        ...
        {validtionMessage && (
          <p className="text-red-500 text-[16px] pt-1 pl-1">
            {validtionMessage}
          </p>
        )}
        <button
