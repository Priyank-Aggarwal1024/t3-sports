...
const ProductSelection = ({ onProductSelect, selectedWarehouse }) => {
  const [productSearch, setProductSearch] = useState("");
  const products = selectedWarehouse?.products || [];
  ...
  const handleSelectProduct = (product) => {
    ...
    if (exists) {
      ...
      setSelectedProducts((prev) =>
        prev.map((p) =>
          p._id === product._id
            ? { ...p.productId, quantity: productQuantity, price: pr }
            : p
        )
      );
    } else {
    ...
    <div className="w-full">
      <div className="w-full flex md:items-center md:gap-6 gap-3 md:flex-row flex-col">
        <label
          ...
          className="text-xl min-w-36 block font-semibold text-gray-800 dark:text-white"
        >
          Select Products
        </label>
        <div className="w-full md:py-4 py-2 md:px-6 px-3 dark:bg-[#121212] bg-gray-300 rounded-[10px] flex justify-between dark:text-white text-black md:text-xl text-md font-normal font-['Inter'] relative">
          ...
          <button
            className="w-12 h-9 rounded-[5px] bg-[#2F60F3] flex items-center justify-center cursor-pointer disabled:bg-[#2f60f3cc] disabled:cursor-default relative z-[1]"
            disabled={!select}
            onClick={() => {
              if (select.quantity < quantities[select.productId._id || 1]) {
                alert("Product quantity should less than current quantity!");
                return;
              }
              handleSelectProduct(select);
            }}
          >
            <img src={plus} alt="plus" />
          </button>
          {productSearch.trim() && (
            <div className="absolute top-full mt-1 rounded-[10px] w-full z-[10] py-3 xs:py-6 dark:bg-black bg-gray-300 shadow-xl dark:border left-0 px-1 xs:px-4">
                  ...
                  filteredProducts.map(
                    (product) =>
                      product.quantity > 0 && (
                        <div
                          key={product._id}
                          className={`py-2 px-1 border xs:flex-nowrap flex-wrap flex justify-between items-center w-full border-gray-600 rounded-md cursor-pointer dark:text-white text-black text-center hover:bg-[#2f60f3cc] ${
                            select &&
                            select?.productId?._id === product.productId._id &&
                            "bg-[#2F60F3] "
                          }`}
                          onClick={() =>
                            setSelect(
                              product.productId._id === select?.productId?._id
                                ? null
                                : product
                            )
                          }
                          ...
                          <div className="flex w-full items-center gap-1">
                            ...
                            <div className="flex items-center w-full justify-center gap-2">
                              <input
                                ...
                                value={quantities[product.productId._id] || 1} // Default value to 1
                                onChange={(e) =>
      ...
      <div className="flex flex-col gap-2 lg:mt-12 md:mt-6 mt-3">
        ...
        {selectedProducts.length > 0 ? (
          selectedProducts.map((product) => (
            <div
              key={product._id}
              className="lg:p-8 md:p-6 xs:flex-nowrap flex-wrap gap-2 xs:gap-1 xs:p-4 p-1 border flex justify-between items-center w-full border-gray-600 rounded-xl dark:bg-[#121212] bg-gray-300 dark:text-white text-black text-center"
            >
...
ProductSelection.propTypes = {
  ...
  selectedWarehouse: PropTypes.oneOfType([
    PropTypes.shape({
      products: PropTypes.arrayOf(PropTypes.object).isRequired, // Replace object with a more specific shape if possible
    }).isRequired,
