...
export const OrdersProvider = ({ children }) => {
  ...
  const avereageOrderValue = () => {
    ...
    const weeklyOrders = orders.filter(order => order.dateOfOrder >= startOfWeek && order.dateOfOrder <= today.toISOString().split("T")[0]);

    // Calculate total revenue for the week
    const totalWeeklyRevenue = weeklyOrders.reduce((sum, order) => sum + parseFloat(order.totalAmount), 0);
    setAov(weeklyOrders.length > 0 ? (totalWeeklyRevenue / weeklyOrders.length) : 0);

  }
  useEffect(() => {
    const today = new Date().toISOString().split("T")[0];
    const todayOrders = orders? orders.filter(order => {
      return (new Date( order.dateOfOrder).toISOString().split("T")[0]) === today
    }):[];
    setTodayOrders(todayOrders.length);
    setTodayRevenue(() => todayOrders.reduce((sum, order) => sum + parseFloat(order.totalAmount), 0));
    avereageOrderValue();
  }, [orders])
  ...
  return (
    <OrdersContext.Provider value={{ orders, loading, error, fetchOrders, todayOrders, todayRevenue, aov }}>
      {children}
