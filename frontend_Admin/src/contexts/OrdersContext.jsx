...
export const OrdersProvider = ({ children }) => {
  ...
  const weeklyOrders = orders.filter(order => order.order_date >= startOfWeek && order.order_date <= today.toISOString().split("T")[0]);
  
  // Calculate total revenue for the week
  const totalWeeklyRevenue = weeklyOrders.reduce((sum, order) => sum + parseFloat(order.order_amount), 0);
...
setTodayRevenue(()=>todayOrders.reduce((sum, order) => sum + parseFloat(order.order_amount), 0));
avereageOrderValue();
  },[orders])
