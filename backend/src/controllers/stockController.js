...
const viewAllStocks = async (req, res) => {
  try {
...
const transferStock = async (req, res) => {
  const { fromWarehouseId, toWarehouseId, productId, size, colour, quantity } =
    req.body;
  ...
  try {
    ...
    if (!fromStock || fromStock.quantity < quantity) {
      return res
        .status(400)
        .json({ message: "Insufficient stock in the source warehouse" });
    }
...
module.exports = { transferStock, updateStock, viewAllStocks };
